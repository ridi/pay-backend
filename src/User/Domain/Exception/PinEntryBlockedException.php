...
class PinEntryBlockedException extends \Exception
{
    ...
    public function __construct(BaseAbuseBlockPolicy $policy, int $remaining_period_for_unblock)
    {
        ...
        $remaining_period_for_unblock_in_minute = ($remaining_period_for_unblock % TimeUnitConstant::SEC_IN_MINUTE === 0)
            ? ($remaining_period_for_unblock / TimeUnitConstant::SEC_IN_MINUTE)
            : floor($remaining_period_for_unblock / TimeUnitConstant::SEC_IN_MINUTE) + 1;
